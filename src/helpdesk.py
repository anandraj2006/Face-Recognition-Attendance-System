...
class Help_desk:
    def __init__(self, root):
        ...
        img_help = Image.open(os.path.join(PICTURES_DIR, "helpdesk1.jpeg"))
        ...
        Label(left_frame, text="We are here to help you", font=("Segoe UI", 14, "bold"), bg="white").pack()

        right_frame = Frame(container, bg="white")
        right_frame.place(x=420, y=10, width=820, height=400)

        Label(right_frame, text="Common Issues", font=("Segoe UI", 18, "bold"), fg="#0b3d91", bg="white").place(x=10, y=5)
        ...
        Label(right_frame, text=issues, font=("Segoe UI", 12), bg="white", justify=LEFT).place(x=10, y=40)

        Label(right_frame, text="Issue Category", font=("Segoe UI", 12, "bold"), bg="white").place(x=10, y=170)
        self.issue_category = ttk.Combobox(right_frame, state="readonly", width=30, font=("Segoe UI", 11))
        self.issue_category["values"] = ("Camera Issue", "Face Detection Problem", "Attendance Issue", "Training Error", "CSV / Database Issue", "Other")
        self.issue_category.current(0)
        self.issue_category.place(x=10, y=200)

        Label(right_frame, text="Describe your issue", font=("Segoe UI", 12, "bold"), bg="white").place(x=10, y=230)
        self.issue_text = Text(right_frame, width=60, height=4.2, font=("Segoe UI", 11), bd=2, relief=GROOVE)
        self.issue_text.place(x=10, y=260)

        Button(right_frame, text="Send via Email", font=("Segoe UI", 12, "bold"), bg="#2563eb", fg="white", cursor="hand2", command=self.send_email).place(x=10, y=360, width=180, height=40)
        Button(right_frame, text="Send via WhatsApp", font=("Segoe UI", 12, "bold"), bg="#22c55e", fg="white", cursor="hand2", command=self.send_whatsapp).place(x=200, y=360, width=200, height=40)

        contact_frame = Frame(right_frame, bg="#f9fafb", bd=2, relief=RIDGE)
        contact_frame.place(x=520, y=40, width=270, height=160)
        Label(contact_frame, text="Contact Support", font=("Segoe UI", 16, "bold"), bg="#f9fafb").pack(pady=10)
        email_lbl = Label(contact_frame, text="📧 anandraj.141520@gmail.com", fg="blue", bg="#f9fafb", cursor="hand2")
        email_lbl.pack()
        email_lbl.bind("<Button-1>", lambda e: webbrowser.open("https://mail.google.com/mail/?view=cm&fs=1&to=anandraj.141520@gmail.com"))
        Label(contact_frame, text="📞 +91 74548 30370", bg="#f9fafb").pack(pady=5)
        whatsapp_lbl = Label(contact_frame, text="💬 WhatsApp Support", fg="green", bg="#f9fafb", cursor="hand2")
        whatsapp_lbl.pack()
        whatsapp_lbl.bind("<Button-1>", lambda e: webbrowser.open("https://wa.me/917454830370"))
    ...
    def send_email(self):
        ...
        body = urllib.parse.quote(f"Issue Category: {category}\n\nIssue Description:\n{issue}")
        gmail_url = f"https://mail.google.com/mail/?view=cm&fs=1&to=anandraj.141520@gmail.com&subject={subject}&body={body}"
    ...
    def send_whatsapp(self):
        ...
        if not issue:
            messagebox.showerror("Error", "Please describe your issue")
            return
        message = urllib.parse.quote(f"Issue Category: {category}\n\nIssue:\n{issue}")
        webbrowser.open(f"https://wa.me/917454830370?text={message}")
