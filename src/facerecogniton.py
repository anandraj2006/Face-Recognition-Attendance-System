...
class FaceRecognition:
    def __init__(self, root):
        ...
        title_lbl = Label(bg_img, text="ATTENDANCE MANAGEMENT SYSTEM", font=("times new roman", 30, "bold"), bg="white", fg="purple")
        title_lbl.pack(fill=X)

        info_frame = LabelFrame(self.root, bd=2, relief=RIDGE, text="Recognition Status", font=("times new roman", 12, "bold"), bg="white")
        info_frame.place(x=20, y=220, width=400, height=530)

        self.status_label = Label(info_frame, text="Status: Camera Ready", font=("Segoe UI", 16, "bold"), fg="blue", bg="white")
        self.status_label.pack(pady=30)

        self.name_label = Label(info_frame, text="Name: ---", font=("Segoe UI", 14), bg="white")
        self.name_label.pack(pady=10)

        self.time_label = Label(info_frame, text="Time: ---", font=("Segoe UI", 14), bg="white")
        self.time_label.pack(pady=10)

        Button(info_frame, text="Take Attendance", font=("times new roman", 14, "bold"), bg="green", fg="white", command=self.start_camera).pack(pady=20, ipadx=20)
        Button(info_frame, text="Stop Camera", font=("times new roman", 14, "bold"), bg="red", fg="white", command=self.stop_camera).pack(pady=10, ipadx=30)

        # Load face cascade from cv2 built-in data (most reliable)
        self.face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
        self.recognizer = cv2.face.LBPHFaceRecognizer_create()

        # Load the trained classifier from src/models/
        classifier_path = os.path.join(MODELS_DIR, "classifier.xml")
        try:
            self.recognizer.read(classifier_path)
        except:
            print(f"classifier.xml not found at {classifier_path}")
    ...
    def mark_attendance(self, student_id, name):
        ...
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="piddii51242", database="mydb")
            cur = conn.cursor()
            cur.execute("SELECT * FROM attendance WHERE attendance_id=%s AND date=%s", (student_id, today))
            if cur.fetchone():
