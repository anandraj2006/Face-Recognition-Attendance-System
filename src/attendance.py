...
class Attendance:
    def __init__(self, root):
        ...
        img_left = Image.open(os.path.join(PICTURES_DIR, "attendance1.jpeg"))
        ...
        date_entry.bind("<1>", self.open_calendar)
        ...
        self.attendance_status = ttk.Combobox(left_inside_frame, width=20, font=("times new roman", 12), state="readonly")
        ...
        # buttons
        Button(left_inside_frame, text="Import csv", cursor="hand2", font=("times new roman", 12, "bold"), bg="royalblue3", fg="white", command=self.import_csv).place(x=7, y=180, width=110, height=35)
        Button(left_inside_frame, text="Export csv", cursor="hand2", font=("times new roman", 12, "bold"), bg="royalblue3", fg="white", command=self.export_csv).place(x=125, y=180, width=110, height=35)
        Button(left_inside_frame, text="Update", cursor="hand2", font=("times new roman", 12, "bold"), bg="royalblue3", fg="white", command=self.update_attendance).place(x=243, y=180, width=90, height=35)
        Button(left_inside_frame, text="Reset", cursor="hand2", font=("times new roman", 12, "bold"), bg="royalblue3", fg="white", command=self.reset_fields).place(x=341, y=180, width=90, height=35)
        Button(left_inside_frame, text="Mark Attendance", cursor="hand2", font=("times new roman", 12, "bold"), bg="royalblue3", fg="white", command=self.mark_attendance).place(x=439, y=180, width=130, height=35)
        ...
        table_scroll_y.config(command=self.Attendancereport.yview)
    ...
    def create_attendance_table(self):
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="piddii51242", database="mydb")
            cur = conn.cursor()
            cur.execute("""
        ...
        try:
            ...
            with open(file, newline="", encoding="utf-8-sig") as f:
                ...
                for row in reader:
                    ...
                    """, (row[0], row[1], row[2], row[3], str(row[4]), row[5], row[6], row[7]))
            conn.commit()
            conn.close()
            self.fetch_data()
    ...
    def fetch_data(self):
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="piddii51242", database="mydb")
            cur = conn.cursor()
            cur.execute("SELECT attendance_id, roll, name, department, time, date, attendance, source FROM attendance")
            rows = cur.fetchall()
    ...
    def export_csv(self):
        try:
            file = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv")], title="Save Attendance CSV")
            if not file:
                return
            conn = mysql.connector.connect(host="localhost", user="root", password="piddii51242", database="mydb")
            cur = conn.cursor()
            ...
            with open(file, mode="w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["attendance_id", "roll", "name", "department", "time", "date", "attendance", "source"])
                for row in rows:
                    writer.writerow(row)
            conn.close()
    ...
    def update_attendance(self):
        ...
        try:
            ...
            cur.execute("""
                UPDATE attendance
                SET roll = %s, name = %s, department = %s, attendance = %s, time = %s, source = 'Manual'
                WHERE attendance_id = %s
                """, (self.rollno_entry.get().strip(), self.name_entry.get().strip(), self.department_entry.get().strip(), status, current_time, self.selected_attendance_id))
            if cur.rowcount == 0:
                messagebox.showerror("Update Failed", "No record was updated. Please reselect the row.", parent=self.root)
                conn.close()
                return
            conn.commit()
            conn.close()
    ...
    def mark_attendance(self):
        ...
        try:
            conn = mysql.connector.connect(host="localhost", user="root", password="piddii51242", database="mydb")
            cur = conn.cursor()
            cur.execute("SELECT * FROM attendance WHERE attendance_id=%s AND date=%s", (attendance_id, current_date))
            if cur.fetchone():
                messagebox.showwarning("Duplicate", "Attendance already marked for today", parent=self.root)
                conn.close()
                return
            cur.execute("""
            ...
            """, (attendance_id, roll, name, department, current_time, current_date, status))
            conn.commit()
            conn.close()
            self.fetch_data()
